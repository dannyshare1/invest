# -*- coding: utf-8 -*-
"""
news_pipeline.py  r18-fix  (2025-08-07)
──────────────────────────────────────
依赖： httpx · python-dateutil · beautifulsoup4 · lxml
Secrets：TUSHARE_TOKEN（可选；用作兜底和新闻 API）
输出： keywords_used.txt · news_all.csv · pipeline.log · errors.log
"""
...
async def get_etf_names(ts_code:str)->list[str]:
    try:
        return await eastmoney_top10(ts_code)
    except Exception as e:
        log_err(f"ETF {ts_code} eastmoney {e}")
    try:
        return await tushare_top10(ts_code)
    except Exception as e:
