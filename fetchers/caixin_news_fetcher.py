...
# -*- coding: utf-8 -*-
"""
fetchers/caixin_news_fetcher.py
---------------------------------
抓取财新网最新资讯，并将首段作为摘要写入列表
"""
...
FEED_URL = "https://rsshub.app/caixin/latest"
REQUEST_TIMEOUT = 10
HEADERS = {
    "User-Agent": (
...
def _first_paragraph(url: str) -> str:
    ...
    container = None
    for candidate in [
        {"name": "div", "class_": lambda x: x and "textblock" in x},
        {"name": "div", "class_": lambda x: x and "article" in x},
        {"name": "div", "class_": lambda x: x and "content" in x},
    ]:
        container = soup.find(**candidate)
        if container:
            break
    if not container:
        container = soup
